...
export class ComponentScanner {
    ...
    async discover(options) {
        ...
        try {
            ...
            const configPath = options.tsconfigPath ?? path.join(options.rootDir, "tsconfig.json");
            ...
            errors.push({
                ...
                error: `Failed to initialize scanner: ${error instanceof Error ? error.message : String(error)}`,
            });
        }
    ...
    shouldScanFile(sourceFile, options) {
        ...
        if (options.include && options.include.length > 0) {
            const included = options.include.some((pattern) => filePath.includes(pattern));
            if (!included)
                return false;
        }
        // Check excludes
        if (options.exclude && options.exclude.length > 0) {
            const excluded = options.exclude.some((pattern) => filePath.includes(pattern));
            if (excluded)
                return false;
        }
        return true;
    ...
    hasJSXReturnType(node) {
        ...
        if (signature) {
            ...
            if (typeString.includes("JSX.Element") ||
        ...
        const visitor = (node) => {
            if (ts.isPropertyAccessExpression(node) &&
                ...
                if (ts.isBinaryExpression(current) &&
                    ...
                    if (ts.isArrowFunction(rightSide) ||
                        ...
                        const isComponent = ts.isArrowFunction(rightSide) ||
                            ts.isFunctionExpression(rightSide)
    ...
    extractComponentMetadataBase(node, sourceFile) {
        ...
        if ((ts.isFunctionDeclaration(node) ||
            ...
            this.checker) {
            const firstParam = ts.isFunctionDeclaration(node) ||
                ts.isArrowFunction(node) ||
                ts.isFunctionExpression(node)
                ? node.parameters[0]
                : ts.isVariableStatement(node) &&
     ...
     */
    extractDefaultsFromTypeLiteral(typeLiteral, defaultValues) {
            ...
            if (ts.isVariableStatement(node) || ts.isVariableDeclarationList(node)) {
                ...
                for (const declaration of declarations) {
                    if (declaration.initializer &&
                        ...
                        this.extractDefaultsFromBindingPattern(declaration.name, defaultValues);
                    }
                }
            }
            ts.forEachChild(node, visitor);
    ...
    evaluateDefaultValue(initializer) {
        ...
        if (ts.isArrayLiteralExpression(initializer)) {
            return initializer.elements.map((element) => this.evaluateDefaultValue(element));
        }
        ...
        if (ts.isObjectLiteralExpression(initializer)) {
            ...
            for (const property of initializer.properties) {
                if (ts.isPropertyAssignment(property) &&
                    ...
                    obj[property.name.text] = this.evaluateDefaultValue(property.initializer);
                }
            }
            return obj;
        }
