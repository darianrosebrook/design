...
export function CanvasArea() {
  ...
  const { objects, selectedId, setSelectedId, setContextMenu, updateObject, activeTool, canvasBackground } = useCanvas()

  const [isDragging, setIsDragging] = useState(false)
  const [isResizing, setIsResizing] = useState(false)
  const [resizeHandle, setResizeHandle] = useState<ResizeHandle | null>(null)
  const [dragStart, setDragStart] = useState({ x: 0, y: 0 })
  const [objectStart, setObjectStart] = useState({ x: 0, y: 0, width: 0, height: 0 })
  ...
  const handleObjectMouseDown = (e: React.MouseEvent, obj: CanvasObject) => {
    ...
    setObjectStart({ x: obj.x, y: obj.y, width: obj.width, height: obj.height })
  }

  const handleResizeMouseDown = (e: React.MouseEvent, handle: ResizeHandle, obj: CanvasObject) => {
    e.stopPropagation()
    setIsResizing(true)
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isDragging) {
        ...
        switch (resizeHandle) {
          case "se":
            updates = { width: objectStart.width + dx, height: objectStart.height + dy }
            break
          case "sw":
            updates = { x: objectStart.x + dx, width: objectStart.width - dx, height: objectStart.height + dy }
            break
          case "ne":
            updates = { y: objectStart.y + dy, width: objectStart.width + dx, height: objectStart.height - dy }
            break
          case "nw":
            updates = {
              x: objectStart.x + dx,
          ...
          case "s":
            ...
            break
          case "n":
            updates = { y: objectStart.y + dy, height: objectStart.height - dy }
            break
        }
  ...
  const renderResizeHandles = (obj: CanvasObject) => {
    ...
    return handles.map((handle) => {
      ...
      return <div key={handle} style={style} onMouseDown={(e) => handleResizeMouseDown(e, handle, obj)} />
    })
  }
        ...
        return (
          <div
            ...
            style={{
              ...
              backgroundColor: obj.fill,
              borderRadius: obj.cornerRadius,
              border: isSelected ? "2px solid #4a9eff" : "1px solid rgba(255,255,255,0.1)",
            }}
            onClick={handleClick}
            onMouseDown={(e) => handleObjectMouseDown(e, obj)}
            onContextMenu={(e) => handleContextMenu(e, obj.id)}
    ...
    switch (canvasBackground) {
      case "dot-grid":
        return {
          ...
          backgroundImage: "radial-gradient(circle at center, rgba(255, 255, 255, 0.15) 1px, transparent 0)",
  ...
  return (
    ...
    >
      {/* Canvas content */}
      <div className="relative w-full h-full p-8">{objects.map((obj) => renderObject(obj))}</div>
