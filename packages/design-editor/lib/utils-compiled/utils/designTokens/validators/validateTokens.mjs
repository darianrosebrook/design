...
const SCHEMA_PATH = path.join(PROJECT_ROOT, 'ui', 'designTokens', 'designTokens.schema.json');
...
function initializeValidator() {
    ...
    if (!fs.existsSync(SCHEMA_PATH)) {
        console.warn('[validation] Schema file not found. Run schema generation first.');
        return null;
    }
...
export function validateTokenFile(filePath) {
    ...
    try {
        ...
        if (!isSchemaValid) {
            ...
            result.errors.push(...(schemaValidator.errors || []).map((error) => ({
                type: 'schema',
...
function performCustomValidations(tokens, filePath) {
    ...
    errors.push(...circularRefs.map((cycle) => ({
        type: 'circular-reference',
        path: cycle.join(' -> '),
...
if (require.main === module) {
    const targetPath = process.argv[2] || path.join(PROJECT_ROOT, 'ui', 'designTokens');
