...
function loadTokenTree() {
    ...
    if (composed && (composed.core || composed.semantic))
        return composed;
...
function pickModeValue(node, mode) {
    ...
    if (paths && typeof paths === 'object') {
        if (mode === 'light' && typeof paths.light === 'string')
            return paths.light;
        if (mode === 'dark' && typeof paths.dark === 'string')
            return paths.dark;
    }
    return node?.$value;
}
function extractRefsFromString(str) {
    ...
    const refs = [];
    let match;
    while ((match = REF_PATTERN.exec(str)) !== null)
        refs.push(match[1]);
    return refs;
}
    ...
    function resolveForMode(modeArg) {
        ...
        function resolveValue(raw) {
            ...
            if (Array.isArray(raw))
                return raw.map((v) => resolveValue(v));
...
function generateReport(tree) {
    ...
    const semanticPaths = extractTokenPaths(tree.semantic || {}).map((p) => `semantic.${p}`);
...
function main() {
    ...
    if (!tree || (!tree.core && !tree.semantic)) {
        console.error('[inspect] No tokens found. Run npm run tokens:compose first.');
        process.exit(1);
    }
