 ...
 */
import { parseCssColorToRgb, formatHsl, formatOklch, } from "../../helpers/colorFormat";
import { rgbToHex, rgbToHsl, rgbToOklch } from "../../helpers/color";
/**
...
export const builtInTransforms = [
    ...
    // Color transform: normalize color formats
    {
        ...
        apply: (value, ctx) => {
            ...
            if (!rgb)
                return value; // leave unknown formats intact
            switch (target) {
                case "rgb":
    ...
    // Typography transform: convert typography objects to CSS
    {
        ...
        apply: (value) => {
            if (typeof value === "object" && value !== null) {
                ...
                const parts = [];
                if (typo.fontFamily)
                    parts.push(`font-family: ${typo.fontFamily}`);
                if (typo.fontSize)
                    parts.push(`font-size: ${typo.fontSize}`);
                if (typo.fontWeight)
