 ...
 */
function buildName(prefix, path, nameCase, cssVarPrefix) {
...
function walk(obj, path, callback) {
    for (const [key, value] of Object.entries(obj)) {
        ...
        if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
            walk(value, currentPath, callback);
        }
        else {
 ...
 * ```
 */
export function generate(tokenConfig, tokens, config) {
    const cssVars = {};
    const { prefix } = tokenConfig;
    walk(tokenConfig.tokens, [], (path, raw, node) => {
        const name = buildName(prefix, path, config.nameCase ?? 'kebab', config.cssVarPrefix ?? '--');
        const ctx = {
            path: path.join('.'),
            node,
            tokens,
        ...
        if (config.output === 'ref-map') {
            ...
            const refValue = typeof raw === 'string' ? String(raw) : `${prefix}.${path.join('.')}`;
 ...
 */
export const validateTokenPath = (tokenPath, designTokens) => {
    try {
 ...
 */
export function formatAsCSS(cssVars, selector = ':root') {
 ...
 */
export const getAvailableTokenPaths = (designTokens, prefix = '') => {
    const paths = [];
    for (const [key, value] of Object.entries(designTokens)) {
        ...
        if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
            if ('$value' in value) {
                ...
                paths.push(...getAvailableTokenPaths(value, currentPath));
            }
        }
