...
const SCHEMA_OUTPUT_PATH = path.join(PROJECT_ROOT, "ui", "designTokens", "designTokens.schema.json");
/**
 ...
 *
 * @returns JSON schema for design token objects
 */
 ...
 *
 * @returns JSON schema for design token groups
 */
 ...
 *
 * @returns Record of type-specific JSON schemas
 */
 ...
 *
 * @returns Complete JSON schema for design tokens
 */
...
function generateSchema() {
    ...
    console.log(`[schema] Generated schema at ${path.relative(PROJECT_ROOT, SCHEMA_OUTPUT_PATH)}`);
    console.log(`[schema] Schema supports ${W3C_TYPES.length} W3C types and ${CUSTOM_TYPES.length} custom types`);
    return schema;
}
