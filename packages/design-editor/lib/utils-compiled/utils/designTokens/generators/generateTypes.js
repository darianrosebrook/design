...
function generateTokenTypes() {
    const coreTokensPath = path.join(PROJECT_ROOT, "ui", "designTokens", "core.tokens.json");
    const semanticTokensPath = path.join(PROJECT_ROOT, "ui", "designTokens", "semantic.tokens.json");
    let allPaths = [];
    ...
    if (fs.existsSync(semanticTokensPath)) {
        ...
        const semanticPaths = extractTokenPaths(semanticTokens).map((p) => `semantic.${p}`);
 ...
 *
 * @returns Generated type definitions string
 */
function generateTypes() {
    ...
    console.log(`[types] Generated types at ${path.relative(PROJECT_ROOT, TYPES_OUTPUT_PATH)}`);
    console.log(`[types] Generated ${typeContent.split("|").length - 1} token paths`);
    return typeContent;
}
