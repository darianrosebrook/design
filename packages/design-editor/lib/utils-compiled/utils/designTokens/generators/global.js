 ...
 */
import { PATHS, readTokenFile, writeOutputFile, tokenPathToCSSVar, formatCSSBlock, generateBanner, logSummary, } from '../core/index';
/**
 * Walk token tree and collect CSS custom properties
 */
function collectTokens(obj, path = [], context, maps) {
    for (const [key, value] of Object.entries(obj)) {
        ...
        if (value && typeof value === 'object') {
            if ('$value' in value) {
                ...
                const hasThemeExtensions = extensions &&
                ...
                collectTokens(value, currentPath, context, maps);
            }
        }
    }
}
 ...
 */
function processTokenValue(value, context) {
 ...
 */
export function generateGlobalTokens() {
