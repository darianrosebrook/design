 ...
 */
import { PATHS, readTokenFile, writeOutputFile, deepMerge, logSummary, } from '../core/index';
/**
 * Transform token references in an object
 */
function transformReferences(obj, prefixer) {
    ...
    for (const [key, value] of Object.entries(obj)) {
        if (typeof value === 'string') {
            ...
            result[key] = value.replace(/\{([A-Za-z0-9_-]+(?:\.[A-Za-z0-9_-]+)+)\}/g, (_, tokenPath) => `{${prefixer(tokenPath)}}`);
        }
...
function createTokenPrefixer() {
    ...
    return function prefixTokenPath(tokenPath) {
        // Already prefixed
        ...
        const isSemanticToken = semanticPatterns.some((pattern) => pattern.test(tokenPath));
...
export function composeTokens() {
    ...
    const missingFiles = [];
    if (!coreTokens)
        missingFiles.push('core.tokens.json');
    if (!semanticTokens)
        missingFiles.push('semantic.tokens.json');
    if (missingFiles.length > 0) {
        console.error('[tokens] Missing required token files:');
        missingFiles.forEach((file) => console.error(`  - ui/designTokens/${file}`));
        return false;
    }
    ...
    const semanticCount = JSON.stringify(semanticTokens).match(/"\$value"/g)?.length || 0;
    logSummary({
