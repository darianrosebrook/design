...
export const PATHS = {
    ...
    semanticTokens: path.join(PROJECT_ROOT, 'ui', 'designTokens', 'semantic.tokens.json'),
...
export function readTokenFile(filePath) {
    try {
        if (!fs.existsSync(filePath)) {
            console.warn(`[tokens] File not found: ${path.relative(PROJECT_ROOT, filePath)}`);
            return null;
        }
        ...
        if (error instanceof SyntaxError) {
            console.error(`[tokens] JSON parse error in ${path.relative(PROJECT_ROOT, filePath)}:`);
            console.error(`  ${error.message}`);
        }
        else {
            console.error(`[tokens] Error reading ${path.relative(PROJECT_ROOT, filePath)}:`, error);
        }
        return null;
    }
}
 ...
 * ```
 */
export function writeOutputFile(filePath, content, description) {
    try {
        ...
        console.error(`[tokens] Failed to write ${path.relative(PROJECT_ROOT, filePath)}:`, error);
        throw error;
    }
}
/**
 ...
 */
export function tokenPathToCSSVar(tokenPath, prefix = '--') {
    return (prefix +
        tokenPath
 ...
 */
export function formatCSSBlock(selector, properties) {
 ...
 */
export function generateBanner(sourcePath) {
