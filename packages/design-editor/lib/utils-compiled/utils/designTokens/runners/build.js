...
export async function buildTokens() {
    ...
    const timeStr = totalDuration < 1000
        ? `${totalDuration}ms`
        : `${(totalDuration / 1000).toFixed(1)}s`;
    console.log(`  ${successCount}/${results.length} steps completed in ${timeStr}`);
    if (totalErrors === 0) {
 ...
 */
export async function runSteps(stepNames) {
    ...
    for (const stepName of stepNames) {
        if (!availableSteps[stepName]) {
            ...
            console.error(`[build] Available steps: ${Object.keys(availableSteps).join(', ')}`);
            return false;
        }
