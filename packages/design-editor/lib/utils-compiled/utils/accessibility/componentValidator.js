 ...
 */
import { validateColorPair, } from './tokenValidator';
import fs from 'fs';
 ...
 */
function extractComponentColorPairs(tokenFilePath, componentName) {
    const pairs = [];
    try {
        ...
        console.warn(`Warning: Could not parse component tokens for ${componentName}:`, error instanceof Error ? error.message : String(error));
    }
    return pairs;
}
/**
 ...
 */
function inferComponentType(componentName, tokens) {
 ...
 */
function findTokenValue(tokens, key) {
    // Direct key lookup
    if (tokens[key])
        return tokens[key];
 ...
 */
function extractCommonColorPairs(tokens, componentName) {
    const pairs = [];
    // Look for color/background combinations
    const colorKeys = Object.keys(tokens).filter((key) => key.includes('color') ||
        key.includes('Color') ||
        key.includes('foreground'));
    const backgroundKeys = Object.keys(tokens).filter((key) => key.includes('background') ||
 ...
 */
export function validateComponentTokens(tokenFilePath) {
 ...
 */
export async function validateAllComponentTokens(componentsDir) {
 ...
 */
export function generateComponentReport(validations) {
    ...
    if (invalidValidations.length > 0) {
        ...
        invalidValidations.forEach((validation) => {
            ...
            validation.results
                ...
                output += `      Contrast: ${result.contrastRatio.toFixed(2)} (required: ${result.requiredRatio})\n`;
