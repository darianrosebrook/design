 ...
 */
import { contrastRatioHex } from '../helpers/color';
import fs from 'fs';
...
function generateContrastSuggestion(pair, currentRatio) {
    ...
    return (`Contrast ratio ${currentRatio.toFixed(2)} is below required ${requiredRatio}. ` +
...
export function generateAccessibilityReport(report) {
    ...
    output += `   ✅ Passing: ${validPairs} (${((validPairs / totalPairs) *
        100).toFixed(1)}%)\n`;
    output += `   ❌ Failing: ${invalidPairs} (${((invalidPairs / totalPairs) *
        100).toFixed(1)}%)\n\n`;
    ...
    if (passingResults.length > 0) {
        ...
        passingResults.forEach((result, index) => {
            const level = result.contrastRatio >= WCAG_LEVELS.AAA_NORMAL ? 'AAA' : 'AA';
            output += `${index + 1}. ${result.context} - ${result.contrastRatio.toFixed(2)} (${level})\n`;
        });
    }
    return output;
}
...
export async function runAccessibilityValidation(tokensPath) {
    const defaultTokensPath = path.join(process.cwd(), 'ui/designTokens/designTokens.json');
