 ...
 */
export function rgbToCam02JCh(rgb) {
 ...
 */
export function clampToSrgbGamut(rgb) {
    return {
 ...
 */
export function approximateColorTemperature(rgb) {
    ...
    const total = r + g + b;
    if (total === 0)
        return 6500; // Neutral gray
    const rRatio = r / total;
    const gRatio = g / total;
    const bRatio = b / total;
    // Rough temperature estimation
    if (bRatio > 0.4)
        return 10000; // Cool blue
    if (rRatio > 0.4)
        return 2500; // Warm red
    if (gRatio > 0.5)
        return 5500; // Neutral green
