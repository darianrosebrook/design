...
const CORE_COMPONENTS = {
  ...
  TextField: () => import("@paths-design/design-system").then(m => m.TextField),
  NumberField: () => import("@paths-design/design-system").then(m => m.NumberField),
  ColorField: () => import("@paths-design/design-system").then(m => m.ColorField),
  PropertiesPanel: () => import("@paths-design/design-system").then(m => m.PropertiesPanel),
  ...
  ToggleButton: () => import("@paths-design/design-system").then(m => m.ToggleButton),
} as const;
...
/**
 * Initialize the registry with core design system components
 */
 ...
 */
function getCoreComponentMetadata(componentName: string): Omit<ComponentMetadata, 'component'> {
  ...
  return metadataMap[componentName] || {
    name: componentName,
 ...
 */
export function getComponentMetadata(componentName: string): ComponentMetadata | null {
 ...
 */
export function updateComponent(componentId: string, updates: Partial<IngestedComponent>): boolean {
  const id = componentId.toLowerCase();
 ...
 */
export async function loadFromDesignSystemPackage(packageName: string): Promise<IngestedComponent[]> {
  ...
  console.log(`Loading components from ${packageName}...`);

  // Mock components for demonstration
  const mockComponents: IngestedComponent[] = [
    {
      id: `${packageName}-custom-button`,
      name: "CustomButton",
      description: "A custom button from an external library",
      category: "Interactive",
      icon: "🔘",
      defaultProps: {
