 ...
 */
function generateTokenSchema(): JSONSchema {
  return {
    type: "object",
    properties: {
      $value: {
 ...
 */
function generateGroupSchema(): JSONSchema {
  return {
    type: "object",
    properties: {
      $type: {
 ...
 */
function generateTypeSchemas(): Record<string, JSONSchema> {
  return {
    colorToken: {
      allOf: [
        { $ref: "#/$defs/token" },
 ...
 */
function generateCompleteSchema(): JSONSchema {
  const schema = {
...
function generateSchema(): JSONSchema {
  ...
  console.log(
    `[schema] Generated schema at ${path.relative(
      PROJECT_ROOT,
      SCHEMA_OUTPUT_PATH
    )}`
  );
