...
export function colorsAreSimilar(
  ...
  const difference =
    method === "76" ? deltaE76(lab1, lab2) : deltaE2000(lab1, lab2);
 ...
 */
export function describeColorDifference(
  difference: number,
  method: "76" | "2000" = "2000"
): string {
  if (method === "76") {
    // CIE76 thresholds
    if (difference < 1) {return "Not perceptible by human eyes";}
    if (difference < 2) {return "Perceptible through close observation";}
    if (difference < 3.5) {return "Perceptible at a glance";}
    if (difference < 5) {return "More similar than opposite";}
    if (difference < 7) {return "Noticeably different";}
    return "Colors are different";
  } else {
    // CIEDE2000 thresholds (generally more forgiving)
    if (difference < 0.8) {return "Not perceptible by human eyes";}
    if (difference < 1.5) {return "Perceptible through close observation";}
    if (difference < 3) {return "Perceptible at a glance";}
    if (difference < 6) {return "More similar than opposite";}
    if (difference < 12) {return "Noticeably different";}
    return "Colors are different";
  }
}
