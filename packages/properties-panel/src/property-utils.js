 ...
 */
export function getNodeProperty(node, propertyKey) {
    ...
    if (parts.length === 2) {
        ...
        switch (parent) {
            case "frame":
                return node.frame ? node.frame[child] : undefined;
            case "textStyle":
                return node.type === "text" && node.textStyle
                    ? node.textStyle[child]
                    : undefined;
            case "text":
 ...
 */
export function setNodeProperty(node, propertyKey, value) {
    // Handle nested property access (e.g., "frame.x", "textStyle.size")
    const parts = propertyKey.split(".");
    if (parts.length === 1) {
        // Direct property access
 ...
 */
export function getApplicablePropertiesForNode(node) {
 ...
 */
export function validatePropertyValue(value, definition) {
