...
import { ChevronRight, GripVertical, Link2, Plus, Unlink } from "lucide-react";
...
export function PropertiesPanel() {
  ...
  const shouldShowSection = (sectionId: string): boolean => {
    ...
    switch (sectionId) {
      ...
      case "fill":
        return ["rectangle", "circle", "text", "frame"].includes(
          selectedObject.type
        );
      case "stroke":
  ...
  const renderSection = (
    ...
    if (!shouldShowSection(section.id)) {
      return null;
    }
    ...
    return (
      <div key={section.id}>
        {showDropIndicator && draggedSectionIndex !== index && (
          <div className="h-0.5 bg-blue-500 mx-1 mb-2" />
        )}
        <div
          draggable
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <Input
                        ...
                        className="h-8 text-sm placeholder:text-foreground/60"
                      />
                    </div>
                    <div>
                      <Input
