  ...
  describe("Valid Paths", () => {
    ...
    it("resolves paths correctly", () => {
      ...
      expect(result.resolvedPath).toBe(
        path.join(workspaceRoot, "design", "home.canvas.json")
      );
    });
  });
  ...
  describe("Cross-Platform", () => {
    it("normalizes Windows-style paths on Unix", () => {
      if (os.platform() !== "win32") {
        ...
        // So "design\\home.canvas.json" is treated as a single filename with backslashes
        // This will fail pattern matching as it doesn't match the expected format
        const result = validator.validate("design/home.canvas.json");
        expect(result.valid).toBe(true);
      } else {
        // On Windows, test that backslashes work
        const result = validator.validate("design\\home.canvas.json");
        expect(result.valid).toBe(true);
      }
    });
