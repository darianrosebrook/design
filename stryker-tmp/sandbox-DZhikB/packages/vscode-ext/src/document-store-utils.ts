...
export function findNodeById(
  ...
  for (const artboard of document.artboards) {
    ...
    const children = Array.isArray(artboard.children) ? artboard.children : [];
    if (children.length > 0) {
      const result = findNodeInChildren(children, nodeId, artboard.id, null, 1);
      if (result) {
        return result;
      }
    }
...
export function getNodesByType(
  ...
  for (const artboard of artboards) {
    ...
    const children = Array.isArray(artboard.children) ? artboard.children : [];
    if (children.length > 0) {
      collectNodesByType(children, nodeType, artboard.id, null, 1, results);
    }
  }
